...
@section('title-block', 'Администраторы')
    ...
    <div class="admin-list">
        ...
        <ul>
            @foreach($admins as $admin)
                <li>
                    {{ $admin->surname }} {{ $admin->name }} {{ $admin->patronymic }} ({{ $admin->email }})
                    <a href="{{ route('edit-admin-page', $admin->id) }}" class="edit-admin">Редактировать</a>
                    <form action="{{ route('delete-admin', $admin->id) }}" method="POST" onsubmit="return confirm('Вы уверены, что хотите удалить этого администратора?');">
                        @csrf
                        @method('DELETE')
        ...
        <a href="{{ route('add-admin-page') }}" class="add-admin">Добавить администратора</a>
    </div>
@endsection
